...
def researcher_summary(pubs,name,yr_range=None):
    ...
    for yr in sorted(rdf["Year"].dropna().unique()):
        ...
        if not yr_df.empty:
            top5_by_year[int(yr)]=yr_df[["Title","Journal","Year",FC,"JUFO Level"]].copy()
    top5=rdf.nlargest(5,FC)[["Title","Journal","Year",FC,"JUFO Level","Open Access","DOI"]].copy() if FC in rdf.columns else pd.DataFrame()
...
def cluster_summary(pubs,cl,yr_range=None):
    ...
    for yr in sorted(edf["Year"].dropna().unique()):
        ...
        if not yr_df.empty:
            top5_by_year[int(yr)]=yr_df[["Title","Warwick Researcher","Journal","Year",FC,"JUFO Level"]].copy()
    top5=edf.nlargest(5,FC)[["Title","Warwick Researcher","Journal","Year",FC,"JUFO Level"]].copy() if FC in edf.columns else pd.DataFrame()
...
def main():
    ...
    *:not([class*="icon"]):not([data-testid*="icon"]):not(.material-symbols-rounded):not(.e1nzilvr5){font-family:'Roboto',sans-serif;}
    h1,h2,h3,h4,h5,h6,p,span,div,label,td,th,li,input,textarea,button,[data-testid="stMarkdownContainer"]{font-family:'Roboto',sans-serif;}
    ...
    with st.sidebar:
        ...
        scival_file=st.file_uploader("SciVal Export (.xlsx)",type=["xlsx"])
        researchers_file=st.file_uploader("Researchers CSV",type=["csv"])
        jufo_file=st.file_uploader("JUFO Export (optional)",type=["csv"])
        st.markdown("---")
        st.markdown("### ⚙️ Settings")
        year_range=st.slider("Publication years",2020,2026,(2021,2026))
        st.caption("Hungyen Lin (56023981300) override applied automatically.")

    if scival_file is None or researchers_file is None:
        st.info("👈 Upload a **SciVal export** and **Researchers CSV** to begin.");return
