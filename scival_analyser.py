...
def main():
    ...
    with st.sidebar:
        ...
        if DEFAULT_SCIVAL.exists():
            st.success(f"Default SciVal loaded", icon="✅")
        ...
        if DEFAULT_RESEARCHERS.exists():
            st.success(f"Default Researchers loaded", icon="✅")
        ...
        jufo_upload=st.file_uploader("JUFO Export (optional)",type=["csv"])
        st.markdown("---")
    ...
    if scival_file is None or researchers_file is None:
        st.info("👈 Upload a **SciVal export** and **Researchers CSV** in the sidebar to begin, or place default files in the app directory.");return
